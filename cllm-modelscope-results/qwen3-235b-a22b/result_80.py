...
def initialize_aircraft(
    ...
    输入:
        aircraft_count(int): 需要初始化的舰载机数量
        support_stand_ids(Sequence[str]): 可供舰载机初始停放的保障站位编号序列

    输出:
        dict[str, dict[str, Any]]: 以舰载机编号为键的舰载机状态字典

    异常:
        ValueError: 舰载机数量小于0或者超过给定保障站位数量时抛出
    """
    if aircraft_count < 0:
    ...
    for index in range(1, aircraft_count + 1):
        ...
        # 处理特定舰载机的到达时间变化
        if aircraft_id == "AC07":
            planned_arrival_time = 8 * 60 + 20  # 08:20
...
def initialize_fixed_power_resources(
    ...
    输入:
        units_per_zone(int): 每个区域内固定供电资源的数量.论文设置为2

    输出:
        list[dict[str, Any]]: 当前可用的固定供电资源列表

    异常:
        ValueError: 资源数量小于0时抛出
    """
    if units_per_zone < 0:
...
def initialize_fixed_hydraulic_resources(
    ...
    输入:
        units_per_zone(int): 每个区域内固定液压资源的数量.论文设置为2

    输出:
        list[dict[str, Any]]: 当前可用的固定液压资源列表
...
def initialize_fixed_fuel_resources(
    ...
    输入:
        units_per_zone(int): 每个区域内固定燃油资源的数量.论文设置为2

    输出:
        list[dict[str, Any]]: 当前可用的固定燃油资源列表
...
def initialize_fixed_oxygen_resources(
    ...
    输入:
        units_per_zone(int): 每个区域内固定供氧资源的数量.论文设置为2

    输出:
        list[dict[str, Any]]: 当前可用的固定供氧资源列表
...
def initialize_mobile_power_resources(
    ...
    输入:
        resource_count(int): 移动供电资源的数量
        initial_location_id(str): 移动供电资源的初始位置编号

    输出:
        list[dict[str, Any]]: 当前可用的移动供电资源列表
...
def initialize_mobile_hydraulic_resources(
    ...
    输入:
        resource_count(int): 移动液压资源的数量
        initial_location_id(str): 移动液压资源的初始位置编号

    输出:
        list[dict[str, Any]]: 当前可用的移动液压资源列表
...
def initialize_mobile_fuel_resources(
    ...
    输入:
        resource_count(int): 移动燃油资源的数量
        initial_location_id(str): 移动燃油资源的初始位置编号

    输出:
        list[dict[str, Any]]: 当前可用的移动燃油资源列表
...
def initialize_mobile_oxygen_resources(
    ...
    输入:
        resource_count(int): 移动供氧资源的数量
        initial_location_id(str): 移动供氧资源的初始位置编号

    输出:
        list[dict[str, Any]]: 当前可用的移动供氧资源列表
...
def initialize_tow_tractors(
    ...
    输入:
        tow_tractor_count(int): 牵引车数量
        initial_location_id(str): 牵引车初始位置编号

    输出:
        list[dict[str, Any]]: 当前可用的牵引车资源列表

    异常:
        ValueError: 牵引车数量小于0时抛出
    """
...
def create_route_segment(
    ...
    输入:
        start_location_id(str): 路径起点编号
        end_location_id(str): 路径终点编号
        distance(float): 路径距离，默认值为1.0

    输出:
        dict[str, Any]: 包含路径编号、起点、终点、距离和通行状态的字典

    异常:
        ValueError: 起点与终点相同或者距离不大于0时抛出
    """
