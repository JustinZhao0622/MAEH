...
def initialize_aircraft(
    ...
    # 如果A5存在于初始保障站位列表中，移除它
    filtered_support_stands = [stand_id for stand_id in support_stand_ids if stand_id != "A5"]

    # 检查移除A5后是否仍然有足够的站位
    if aircraft_count > len(filtered_support_stands):
        raise ValueError("舰载机数量不能超过可用保障站位数量（排除故障站位后）")
    ...
    for index in range(1, aircraft_count + 1):
        ...
        aircraft[aircraft_id] = {
            ...
            "location_id": filtered_support_stands[index - 1],
