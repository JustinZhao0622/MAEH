...
def initialize_aircraft(
    ...
    # 过滤掉不可用的保障站位
    available_support_stand_ids = [stand_id for stand_id in support_stand_ids if stand_id != "B2"]
    if aircraft_count > len(available_support_stand_ids):
    ...
    for index in range(1, aircraft_count + 1):
        ...
        aircraft[aircraft_id] = {
            ...
            "location_id": available_support_stand_ids[index - 1],
...
def create_route_segment(
    ...
    # 如果路径连接到B2站位，则设置为不可通行
    if end_location_id == "B2":
        return {
