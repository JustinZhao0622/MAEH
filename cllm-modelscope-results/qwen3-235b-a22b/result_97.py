...
def initialize_aircraft(
    ...
    # 过滤掉B4站位
    available_stands = [stand_id for stand_id in support_stand_ids if stand_id != "B4"]
    
    if aircraft_count > len(available_stands):
        raise ValueError("舰载机数量不能超过可用保障站位数量（排除故障站位B4后）")
    ...
    for index in range(1, aircraft_count + 1):
        ...
        # 跳过B4站位
        while stand_index < len(support_stand_ids) and support_stand_ids[stand_index] == "B4":
            stand_index += 1
            
        if stand_index >= len(available_stands):
            raise ValueError("没有足够的可用保障站位来放置所有舰载机")
            
        aircraft[aircraft_id] = {
            "aircraft_id": aircraft_id,
            "location_id": support_stand_ids[stand_index],
            "planned_arrival_time": float(8 * 60 + (index - 1) * 3),
            "actual_arrival_time": float(8 * 60 + (index - 1) * 3),
        }
        stand_index += 1
