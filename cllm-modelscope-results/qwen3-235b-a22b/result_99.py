...
def initialize_support_stands() -> dict[str, dict[str, Any]]:
    ...
    输出：
        dict[str, dict[str, Any]]：
        A区6个、B区8个、C区5个保障站位组成的字典，其中B6站位状态为故障停用。
    """
    ...
    for zone, count in stand_counts.items():
        for index in range(1, count + 1):
            ...
            stands[stand_id] = {
                ...
                "kind": "保障站位",
                "zone": zone,
                "status": "故障停用" if stand_id == "B6" else "可用",
            }
    return stands
...
def initialize_aircraft(
    ...
    # 过滤掉故障停用的B6站位
    available_support_stands = [stand_id for stand_id in support_stand_ids if stand_id != "B6"]

    if aircraft_count > len(available_support_stands):
        raise ValueError("舰载机数量不能超过可用保障站位数量")
    ...
    for index in range(1, aircraft_count + 1):
        ...
        aircraft[aircraft_id] = {
            ...
            "location_id": available_support_stands[index - 1],
