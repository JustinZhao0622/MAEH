...
def initialize_runways() -> dict[str, dict[str, Any]]:
    ...
    for runway_id in ("R1", "R2", "R3"):
        is_available = runway_id != "R2"
        runways[runway_id] = {
            "location_id": runway_id,
            "kind": "起飞跑道",
            "zone": None,
            "available": is_available,
            "unavailable_reason": "通行条件不满足" if not is_available else None,
        }
    return runways
...
def create_route_segment(
    ...
    is_blocked = start_location_id == "R2" or end_location_id == "R2"
    return {
        "segment_id": segment_id,
        "start_location_id": start_location_id,
        "end_location_id": end_location_id,
        "distance": float(distance),
        "status": "不可通行" if is_blocked else "可通行",
        "blocked_reason": "通行条件不满足" if is_blocked else None,
    }
