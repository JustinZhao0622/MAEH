    ...
    for zone, count in stand_counts.items():
        for index in range(1, count + 1):
            ...
            stands[stand_id] = {
                ...
                "kind": "保障站位",
                "zone": zone,
                "status": "故障停用" if stand_id == "A2" else "可用",
            }
    return stands
...
def initialize_aircraft(
    ...
    available_stand_ids = [sid for sid in support_stand_ids if sid != "A2"]
    if aircraft_count > len(available_stand_ids):
        raise ValueError("舰载机数量不能超过可用保障站位数量")
    ...
    for index in range(1, aircraft_count + 1):
        ...
        aircraft[aircraft_id] = {
            ...
            "location_id": available_stand_ids[index - 1],
...
def create_route_segment(
    ...
    return {
        ...
        "status": "不可通行" if start_location_id == "A2" or end_location_id == "A2" else "可通行",
        "blocked_reason": "保障站位A2故障停用" if start_location_id == "A2" or end_location_id == "A2" else None,
    }
