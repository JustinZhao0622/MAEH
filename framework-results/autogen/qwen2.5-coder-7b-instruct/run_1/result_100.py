...
def adjust_support_stands(support_stands: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    if "B7" in support_stands:
        del support_stands["B7"]
...
def main():
    ...
    # 初始化舰载机
    aircraft_count = 10
    support_stand_ids = [stand_id for stand_id in support_stands if stand_id != "B7"]
    aircraft = initialize_aircraft(aircraft_count, support_stand_ids)

    # 初始化资源
    fixed_power_resources = initialize_fixed_power_resources(2)
    fixed_hydraulic_resources = initialize_fixed_hydraulic_resources(2)
    fixed_fuel_resources = initialize_fixed_fuel_resources(2)
    fixed_oxygen_resources = initialize_fixed_oxygen_resources(2)
    mobile_power_resources = initialize_mobile_power_resources(2, "L1")
    mobile_hydraulic_resources = initialize_mobile_hydraulic_resources(2, "L1")
    mobile_fuel_resources = initialize_mobile_fuel_resources(2, "L1")
    mobile_oxygen_resources = initialize_mobile_oxygen_resources(2, "L1")
    tow_tractors = initialize_tow_tractors(2, "L1")

    # 创建路径段
    route_segments = [
        create_route_segment("L1", "A1"),
        create_route_segment("A1", "B1"),
        create_route_segment("B1", "C1"),
        create_route_segment("L2", "A2"),
        create_route_segment("A2", "B2"),
        create_route_segment("B2", "C2"),
        create_route_segment("R1", "L1"),
        create_route_segment("R2", "L2"),
        create_route_segment("R3", "L1"),
    ]
