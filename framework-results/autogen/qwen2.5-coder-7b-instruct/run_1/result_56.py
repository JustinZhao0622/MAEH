...
def adjust_runway_status(runway_id: str) -> dict[str, Any]:
    """调整起飞跑道的状态。
    ...
    if runway_id not in ("R1", "R2", "R3"):
        raise ValueError("无效的起飞跑道编号")

    runways = initialize_runways()
    runway = runways.get(runway_id)
    if not runway:
        raise ValueError("起飞跑道不存在")

    runway["status"] = "不可通行"
    runway["blocked_reason"] = "连接通道通行受限"
    return runway
