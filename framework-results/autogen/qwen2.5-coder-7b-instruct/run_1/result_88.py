...
def adjust_aircraft_assignment(
    ...
    for aircraft_id, aircraft_info in aircraft.items():
        if aircraft_info["location_id"] == "A1":
            # 移动A1站位的舰载机到其他站位
            available_stands = [stand_id for stand_id, stand_info in support_stands.items() if stand_id != "A1"]
            if available_stands:
                aircraft_info["location_id"] = random.choice(available_stands)
            else:
                raise ValueError("没有可用的站位")
