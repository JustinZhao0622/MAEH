...
def adjust_support_stands(support_stands: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """调整保障站位，移除保障站位B5。
    ...
    if "B5" in support_stands:
        del support_stands["B5"]
    return support_stands


def main():
    # 初始化资源
    support_tasks = initialize_support_tasks()
    landing_points = initialize_landing_points()
    support_stands = initialize_support_stands()
    runways = initialize_runways()
    aircraft = initialize_aircraft(10, ["A1", "A2", "A3", "A4", "A5", "A6", "B1", "B2", "B3", "B4", "B6", "B7", "B8", "C1", "C2", "C3", "C4", "C5"])
    fixed_power_resources = initialize_fixed_power_resources()
    fixed_hydraulic_resources = initialize_fixed_hydraulic_resources()
    fixed_fuel_resources = initialize_fixed_fuel_resources()
    fixed_oxygen_resources = initialize_fixed_oxygen_resources()
    mobile_power_resources = initialize_mobile_power_resources(5, "L1")
    mobile_hydraulic_resources = initialize_mobile_hydraulic_resources(5, "L1")
    mobile_fuel_resources = initialize_mobile_fuel_resources(5, "L1")
    mobile_oxygen_resources = initialize_mobile_oxygen_resources(5, "L1")
    tow_tractors = initialize_tow_tractors(5, "L1")
    routes = [
        create_route_segment("A1", "L1"),
        create_route_segment("A2", "L1"),
        create_route_segment("A3", "L2"),
        create_route_segment("A4", "L2"),
        create_route_segment("A5", "L3"),
        create_route_segment("A6", "L3"),
        create_route_segment("B1", "L1"),
        create_route_segment("B2", "L1"),
        create_route_segment("B3", "L2"),
        create_route_segment("B4", "L2"),
        create_route_segment("B6", "L3"),
        create_route_segment("B7", "L3"),
        create_route_segment("B8", "L4"),
        create_route_segment("C1", "L2"),
        create_route_segment("C2", "L2"),
        create_route_segment("C3", "L3"),
        create_route_segment("C4", "L3"),
        create_route_segment("C5", "L4"),
    ]

    # 调整保障站位
    support_stands = adjust_support_stands(support_stands)

    # 调整其他相关逻辑

    print(support_stands)
    print(aircraft)
    print(fixed_power_resources)
    print(fixed_hydraulic_resources)
    print(fixed_fuel_resources)
    print(fixed_oxygen_resources)
    print(mobile_power_resources)
    print(mobile_hydraulic_resources)
    print(mobile_fuel_resources)
    print(mobile_oxygen_resources)
    print(tow_tractors)
    print(routes)
