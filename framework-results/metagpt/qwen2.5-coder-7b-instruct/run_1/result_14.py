            ...
            resources.append(
                {
                    ...
                    "mobility": "固定",
                    "zone": zone,
                    "current_location_id": None if zone == "B" and index == 2 else None,
                    "capacity": 1,
                }
            )
    return resources
...
# 添加函数来处理固定燃油资源的故障
def fix_fixed_fuel_resources(resource_ids: Iterable[str]) -> None:
    """标记固定燃油资源为不可用。
...
# 在需要使用固定燃油资源的函数中添加故障处理逻辑
def assign_fuel_to_aircraft(
    aircraft_id: str,
    fuel_resource_id: str,
    ...
    # 检查燃油资源是否可用
    resource = next((res for res in resources if res["resource_id"] == fuel_resource_id), None)
    if resource is not None and resource["current_location_id"] is None:
        fix_fixed_fuel_resources([fuel_resource_id])
