...
def initialize_aircraft(
    ...
    aircraft_id: str = "AC14",
    planned_arrival_time: int = 8 * 60 + 39 * 60 + 4 * 60,
) -> dict[str, dict[str, Any]]:
    """初始化舰载机并依次放置在给定保障站位。
    ...
    for index in range(1, aircraft_count + 1):
        current_aircraft_id = f"AC{index:02d}"
        actual_arrival_time = planned_arrival_time + (index - 1) * 3
    ...
    aircraft[aircraft_id] = {
        ...
        "location_id": support_stand_ids[0],  # 假设AC14的初始位置为第一个保障站位
        "planned_arrival_time": float(planned_arrival_time),
        "actual_arrival_time": float(planned_arrival_time + 4 * 60),  # 延迟4分钟
    }
    return aircraft
