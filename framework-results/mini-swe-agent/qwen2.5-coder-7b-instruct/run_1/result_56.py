...
def initialize_runways() -> dict[str, dict[str, Any]]:
    ...
    return {
        runway_id: {
            ...
            "kind": "起飞跑道",
            "zone": None,
            "status": "不可通行" if runway_id == "R3" else "可通行",
            "blocked_reason": "连接通道通行受限" if runway_id == "R3" else None,
        }
...
def create_route_segment(
    ...
    return {
        ...
        "status": "可通行" if start_location_id != "R3" and end_location_id != "R3" else "不可通行",
        "blocked_reason": "连接通道通行受限" if start_location_id == "R3" or end_location_id == "R3" else None,
    }
