...
def initialize_fixed_oxygen_resources(
    ...
    # Modify the function to reflect the emergency situation
    resources: list[dict[str, Any]] = []
    for zone in ("A", "B", "C"):
        for index in range(1, units_per_zone + 1):
            resource_id = f"FIX-{zone}-OXYGEN-{index:02d}"
            if zone == "B" and index == 1:
                # Simulate the failure of the fixed oxygen resource in zone B
                resources.append(
                    {
                        "resource_id": resource_id,
                        "resource_type": "供氧",
                        ...
                        "zone": zone,
                        "current_location_id": None,
                        "capacity": 0,  # Set capacity to 0 to indicate the resource is unavailable
                    }
                )
            else:
                resources.append(
