...
def initialize_support_tasks() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        if required_resource is None or required_resource != "供电"
    }  # Exclude tasks that require the faulty power resource
...
def initialize_fixed_power_resources(
    ...
    for zone in ("A", "B", "C"):
        for index in range(1, units_per_zone + 1):
            ...
            # Exclude the faulty power resource
            if zone == "A" and index == 2:
                continue
            resources.append(
                {
