...
def initialize_fixed_hydraulic_resources(
    ...
    # Update to handle the fault in MOB-HYDRAULIC-02
    resources: list[dict[str, Any]] = []
    for zone in ("A", "B", "C"):
        for index in range(1, units_per_zone + 1):
            resource_id = f"FIX-{zone}-HYDRAULIC-{index:02d}"
            if index == 1:  # Assume MOB-HYDRAULIC-02 is the first mobile hydraulic resource
                resources.append(
                    {
                        "resource_id": resource_id,
                        "resource_type": "液压",
                        ...
                        "zone": zone,
                        "current_location_id": None,
                        "capacity": 0,  # Set capacity to 0 to indicate the fault
                        "faulty": True
                    }
                )
            else:
