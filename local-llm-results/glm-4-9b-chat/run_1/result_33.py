...
def initialize_fixed_fuel_resources(
    ...
    # Modify the function to handle the emergency situation
    resources: list[dict[str, Any]] = []
    for zone in ("A", "B", "C"):
        for index in range(1, units_per_zone + 1):
            if zone == "C":  # Assuming "MOB-FUEL-02" is in zone C
                ...
                resources.append(
                    {
                        ...
                        "zone": zone,
                        "current_location_id": None,
                        "capacity": 0,  # Marked as unavailable due to failure
                    }
                )
            else:
                resource_id = f"{zone}-FUEL-{index:02d}"
                resources.append(
                    {
                        "resource_id": resource_id,
                        "resource_type": "燃油",
