...
def initialize_mobile_oxygen_resources(
    ...
    # Updated function body to address the emergency situation
    resources: list[dict[str, Any]] = []
    for index in range(1, resource_count + 1):
        # Check if the resource is MOB-OXYGEN-02 and skip it if it's in use
        if index == 2 and initial_location_id == f"MOB-OXYGEN-02":
            continue  # Skip initializing MOB-OXYGEN-02 due to failure
