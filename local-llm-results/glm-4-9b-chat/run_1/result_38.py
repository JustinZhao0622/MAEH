...
def initialize_fixed_oxygen_resources(
    ...
    # Adjusted to reflect the emergency situation
    if units_per_zone < 0:
        raise ValueError("固定供氧资源数量不能小于0")

    resources: list[dict[str, Any]] = []
    for zone in ("A", "B", "C"):
        # Assuming that only one of the resources is affected by the emergency
        for index in range(1, units_per_zone + 1):
            if index == 1:
                ...
                resources.append(
                    {
                        ...
                        "zone": zone,
                        "current_location_id": None,
                        "capacity": 0,  # Assuming capacity is affected by the emergency
                    }
                )
            else:
                resource_id = f"FIX-{zone}-OXYGEN-{index:02d}"
...
# The specific function for initializing mobile oxygen resources needs to account for
# the emergency situation with MOB-OXYGEN-03. Here we simulate that the first mobile
# oxygen resource is affected by the emergency, hence its capacity is set to 0.
...
def initialize_mobile_oxygen_resources(
    ...
    for index in range(1, resource_count + 1):
        ...
        if index == 1:
            # Simulate the emergency situation for the first mobile oxygen resource
            resources.append(
                {
                    "resource_id": resource_id,
                    "resource_type": "供氧",
                    ...
                    "zone": None,
                    "current_location_id": initial_location_id,
                    "capacity": 0,  # Simulating the failure of MOB-OXYGEN-03
                }
            )
        else:
            resources.append(
