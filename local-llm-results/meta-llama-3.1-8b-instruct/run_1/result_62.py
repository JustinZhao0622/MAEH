...
def initialize_support_tasks() -> dict[str, dict[str, Any]]:
    ...
    # 将L1至A5之间的路径SEG-L1-A5设为不可通行
    path_restrictions = {"SEG-L1-A5": "不可通行"}
    task_rows = [
        ("T01", "机位确认", "基础准备", None),
        ("T02", "外观检查", "基础准备", None),
        ("T03", "供电接入", "基础准备", "供电"),
    ...
    return {
        task_id: {
            ...
            "path_restrictions": path_restrictions
        }
...
def create_route_segment(
    ...
    return {
        ...
        "status": "不可通行" if start_location_id == "L1" and end_location_id == "A5" else "可通行",
        "blocked_reason": None,
    }
