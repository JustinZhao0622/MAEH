...
def initialize_aircraft(
    ...
    delayed_departure_time: int = 84,  # 08:21 + 13分钟 = 08:34
    ...
    输入：
        ...
        support_stand_ids（Sequence[str]）：
            可供舰载机初始停放的保障站位编号序列。
        delayed_departure_time（int，Optional）：
            舰载机延迟到达时间（分钟），默认值为84分钟（08:21 + 13分钟）。
    ...
    for index in range(1, aircraft_count + 1):
        ...
        planned_arrival_time = delayed_departure_time + (index - 1) * 3
