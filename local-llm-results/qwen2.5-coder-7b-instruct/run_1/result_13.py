...
def adjust_fuel_resources() -> dict[str, dict[str, Any]]:
    """调整固定燃油资源的状态，使其不可用。
    ...
    for resource in fixed_fuel_resources:
        if resource["resource_id"] == "FIX-B-FUEL-01":
            resource["current_location_id"] = None
            resource["status"] = "不可用"
    return fixed_fuel_resources


# 调用调整燃油资源函数
adjust_fuel_resources()
