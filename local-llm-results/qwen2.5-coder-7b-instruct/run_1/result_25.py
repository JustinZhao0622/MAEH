...
def assign_power_resource_to_tow_tractor(
    tow_tractor_id: str,
    power_resource_id: str,
    resources: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """将移动供电资源分配给牵引车。
    ...
    for resource in resources:
        if resource["resource_id"] == power_resource_id and resource["resource_type"] == "供电":
            resource["current_location_id"] = tow_tractor_id
            break
    return resources
...
if __name__ == "__main__":
    ...
    stands = initialize_support_stands()
    runways = initialize_runways()
    aircraft = initialize_aircraft(10, list(stands.keys()))
    mobile_power_resources = initialize_mobile_power_resources(2, "A1")
    mobile_hydraulic_resources = initialize_mobile_hydraulic_resources(2, "A1")
    mobile_fuel_resources = initialize_mobile_fuel_resources(2, "A1")
    mobile_oxygen_resources = initialize_mobile_oxygen_resources(2, "A1")
    tow_tractors = initialize_tow_tractors(2, "A1")

    # 分配移动供电资源给牵引车
    mobile_power_resources = assign_power_resource_to_tow_tractor("MOB-TOW-01", "MOB-POWER-01", mobile_power_resources)
    mobile_power_resources = assign_power_resource_to_tow_tractor("MOB-TOW-02", "MOB-POWER-02", mobile_power_resources)
