...
def get_available_runways() -> list[str]:
    ...
    available_runways = [runway_id for runway_id, runway_info in runways.items() if runway_id != "R3"]
    return available_runways


def schedule_flights(available_runways: list[str]) -> None:
    """调度飞行任务。
    ...
    aircrafts = initialize_aircraft(3, get_available_runways())
    for aircraft_id, aircraft_info in aircrafts.items():
        location_id = aircraft_info["location_id"]
        if location_id in ("R1", "R2"):
            for task in initialize_support_tasks().values():
                if task["required_resource"] is not None:
                    resource = get_resource_by_type_and_zone(task["required_resource"], location_id)
                    if resource is not None:
                        assign_resource_to_task(resource, task)
        else:
            assign_to_landing_point(aircraft_id, location_id)


def get_resource_by_type_and_zone(resource_type: str, zone: str) -> dict[str, Any]:
    """根据资源类型和区域获取可用资源。
    ...
    # 这里假设有一个函数可以获取特定类型和区域的资源
    return None


def assign_resource_to_task(resource: dict[str, Any], task: dict[str, Any]) -> None:
    """将资源分配给任务。
...
def assign_to_landing_point(aircraft_id: str, location_id: str) -> None:
    """将舰载机分配到着陆点。
