...
def check_runway_availability(runway_id: str) -> bool:
    ...
    return runway_status.get(runway_id, "不可用") == "可用"


def initialize_scheduling() -> None:
    """初始化调度系统，根据当前资源和约束生成调度计划。
    ...
    # 假设当前可用的起飞跑道
    available_runways = [runway_id for runway_id in initialize_runways().keys() if check_runway_availability(runway_id)]

    # 假设当前可用的舰载机
    aircraft = initialize_aircraft(10, list(initialize_support_stands().keys()))

    # 假设当前可用的资源
    resources = {
        "供电": initialize_fixed_power_resources() + initialize_mobile_power_resources(5, "A1"),
        "液压": initialize_fixed_hydraulic_resources() + initialize_mobile_hydraulic_resources(5, "A2"),
        "燃油": initialize_fixed_fuel_resources() + initialize_mobile_fuel_resources(5, "A3"),
        "供氧": initialize_fixed_oxygen_resources() + initialize_mobile_oxygen_resources(5, "A4"),
    }

    # 初始化调度计划
    schedule = {}
    for aircraft_id, aircraft_info in aircraft.items():
        assignment = {"runway_id": available_runways.pop(0), "resources": {}}
        for resource_type, required_resource in aircraft_info.get("required_resource", {}).items():
            assignment["resources"][resource_type] = resources[resource_type].pop(0)
        schedule[aircraft_id] = assignment
