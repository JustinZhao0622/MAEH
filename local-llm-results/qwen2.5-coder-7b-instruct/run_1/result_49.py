...
def check_runway_availability(runway_id: str) -> bool:
    ...
    输出：
        bool：
        如果跑道可用，返回True；否则返回False。
    """

    runways = initialize_runways()
    if runway_id in runways and runways[runway_id].get("status") == "可用":
        return True
    return False
...
def schedule_aircraft(
    ...
    if not check_runway_availability(runway_id):
        raise ValueError("指定的起飞跑道不可用")

    aircraft = initialize_aircraft(1, support_stand_ids)
    if aircraft_id not in aircraft:
        raise ValueError(f"指定的舰载机编号{aircraft_id}不存在")

    resources = initialize_fixed_power_resources() + initialize_fixed_hydraulic_resources() + initialize_fixed_fuel_resources() + initialize_fixed_oxygen_resources()
    if any(required_resources.get(resource_type, 0) > available for resource_type, available in Counter(resource["resource_type"] for resource in resources).items()):
        raise ValueError("资源不足，无法安排起飞")

    schedule_result = {
        "aircraft_id": aircraft_id,
        "runway_id": runway_id,
        "start_time": 9 * 60,
        "end_time": 9 * 60 + 15,  # 假设起飞时间为9:00，持续时间为15分钟
    }
