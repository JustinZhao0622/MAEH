...
def get_available_runways() -> list[str]:
    ...
    available_runways = [runway_id for runway_id, runway_info in runways.items() if runway_id != "R1"]
...
def schedule_flights(available_runways: list[str]) -> None:
    ...
    aircrafts = initialize_aircraft(3, ["A1", "A2", "A3"])
    for aircraft_id, aircraft_info in aircrafts.items():
        selected_runway = None
        for runway_id in available_runways:
            if runway_id not in aircraft_info["current_runway"]:
                selected_runway = runway_id
                break

        if selected_runway:
            aircraft_info["current_runway"] = selected_runway
        else:
            print(f"无法为舰载机 {aircraft_id} 分配起飞跑道")
