...
def get_available_runways() -> list[str]:
    """获取当前可用的起飞跑道。
    ...
    available_runways = [runway_id for runway_id, runway_info in runways.items() if runway_info["zone"] is not None]
    return available_runways


def schedule_takeoff(
    aircraft_id: str,
    runway_id: str,
    ...
    if runway_id not in get_available_runways():
        raise ValueError("起飞跑道不可用")

    for resource_type, resource_id in resource_assignments.items():
        if resource_type == "供电":
            resource = next((resource for resource in initialize_fixed_power_resources() if resource["resource_id"] == resource_id), None)
        elif resource_type == "液压":
            resource = next((resource for resource in initialize_fixed_hydraulic_resources() if resource["resource_id"] == resource_id), None)
        elif resource_type == "燃油":
            resource = next((resource for resource in initialize_fixed_fuel_resources() if resource["resource_id"] == resource_id), None)
        elif resource_type == "供氧":
            resource = next((resource for resource in initialize_fixed_oxygen_resources() if resource["resource_id"] == resource_id), None)
        else:
            raise ValueError(f"无效的资源类型：{resource_type}")

        if resource is None or resource["current_location_id"] is not None:
            raise ValueError("资源分配不匹配")
