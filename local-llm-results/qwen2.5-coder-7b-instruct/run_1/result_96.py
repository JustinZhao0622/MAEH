...
def update_stands_for_emergency(stands: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """更新保障站位，移除B3站位。
    ...
    updated_stands = deepcopy(stands)
    if "B3" in updated_stands:
        del updated_stands["B3"]
    return updated_stands


# 示例调用
if __name__ == "__main__":
    supports = initialize_support_tasks()
    lands = initialize_landing_points()
    stands = initialize_support_stands()
    runways = initialize_runways()
    aircrafts = initialize_aircraft(5, list(stands.keys()))
    powers = initialize_fixed_power_resources()
    hydraulics = initialize_fixed_hydraulic_resources()
    fuels = initialize_fixed_fuel_resources()
    oxygens = initialize_fixed_oxygen_resources()
    tow_tractors = initialize_tow_tractors(5, "A1")

    # 更新保障站位，移除B3站位
    updated_stands = update_stands_for_emergency(stands)

    # 现在updated_stands中不再包含B3站位
    print(updated_stands)
